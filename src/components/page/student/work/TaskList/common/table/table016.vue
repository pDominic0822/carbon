	...
	<div class="contentBox">
		<table class="demo-table">
			...
			<tr v-for="(trItem, trIndex) in trList" :key="trIndex">
				...
				<td>
					<el-button type="success" :disabled="trItem.isChoose" @click="_handChoose(index)">
							{{!trItem.isChoose ? '等待选单' : '已经选单'}}
						</el-button>
				</td>
			</tr>
		</table>
		<el-dialog
			...
			<div class="orderpart fl" v-for="(item, index) in orderList" :key="index">
				...
				<div class="rhdes fl">
					...
					<div class="chooseBox">
						<el-button type="success" :disabled="item.isChoose" @click="handOrder(index)">
							{{!item.isChoose ? '选择' : '已选择'}}
						</el-button>
					</div>
				</div>
			</div>
...
export default {
	...
	methods: {
		init () {
			this.isorgID = this.$storage.get('orgId');
