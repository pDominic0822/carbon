	...
	<div class='largeCon'>
		...
		<div class='contentDiv'>
			...
			<div class="eacMap">
				...
				<!-- 显示业务信息 -->
				<taskNodeName :IsTaskName="IsTaskName" :taskFromPoint='taskFromPoint'></taskNodeName>
				<!-- 线 -->
...
import { flowOTaskLine } from './json/json3';
...
export default {
	...
	methods: {
		...
		onChangeCommonData () {
			...
			this.IsTaskName = this.navArr[0].jurisdictionArr[1].checked;
			// 2、第二个组件
			...
			orderCodeArr.forEach(element => {
				if (flowOTaskLine[element]) {
					taskNodeArr.push(...flowOTaskLine[element]);
				}
			});
			let taskFromPoint = {}; // 业务数组
			// console.log(taskNodeArr);
			let allShowLine = []; // 所有的连线（未区分流类型）
			taskNodeArr.forEach(element => {
				if (element.fromPoint) {
					// --- 业务数据开始
					if (taskFromPoint[element.fromPoint]) {
						taskFromPoint[element.fromPoint].taskList.push({
							taskName: element.taskName,
							color: allPoint[element.fromPoint].color,
							taskIndex: taskFromPoint[element.fromPoint].taskList.length
						});
					} else {
						taskFromPoint[element.fromPoint] = {
							fromPoint: element.fromPoint,
							pointValue: allPoint[element.fromPoint].value,
							taskList: []
						};
						taskFromPoint[element.fromPoint].taskList.push({
							color: allPoint[element.fromPoint].color,
					...
					// 精确点
					allShowLine.push(...element.pointList); // 线处理
				}
			});
			this.taskFromPoint = taskFromPoint;
			// console.log(taskFromPoint);
			// console.log(allShowLine);
			// ============
			// 3、第三个组件
			...
			// 	flowD: '#1590F1'
			// };
			for (let index = 0; index < allShowLine.length; index++) {
				const all = allShowLine[index];
				for (let j = 0; j < flowCodeArr.length; j++) {
					const flow = flowCodeArr[j];
					if (all.flowType.indexOf(flow) !== -1) { // 展示当前
						// showLineArr.push(all);
						showLineArr.push({
							...all,
							color: allPoint[all.source].color
						});
						break;
					}
				}
