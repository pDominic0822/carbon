	...
	<div class="login">
		...
		<div class="mask-contents">
			...
			<div class="mt30 inter-btn clearfix" v-if="isLogin === 'loginPage' || isLogin === 'loginMessage' || isLogin === 'registerPage' || isLogin === 'forgetPassword'">
				...
				<span v-show="noteNumber" class="fr login-span" :class="{'active': isLogin === 'loginMessage'}" @click="_handleLogin('loginMessage')">
					短信登陆
				</span>
			</div>
			<!-- 用户密码登陆 -->
			...
			<template v-if="isLogin === 'loginMessage'">
				<el-form class="mt20" :model="ruleForm" :rules="rules" ref="ruleForm"  size="small">
					...
					<el-row :gutter="20" class="">
						...
						<el-col :span="8">
							<div class="sub-button"  @click="ruleForm.cellPhone && _handleVeriCode()" :class="{'sub-active': !isCode}">{{text}}</div>
						</el-col>
					</el-row>
				</el-form>
			</template>
			...
			<div class="mt20">
				<el-button v-if="isLogin === 'loginPage'" :disabled="(!isdata || !ruleForm.grapCode || !ruleForm.password)" style="width:100%" type="primary" @click="_pageLogin()">账户登陆</el-button>
				...
				<el-button v-if="isLogin === 'registerPage'" :disabled="(!isdata || !ruleForm.password || !ruleForm.grapCode || !ruleForm.statementState)" style="width:100%" type="primary" @click="_pageRegister()">注册</el-button>
				<template v-if="isLogin === 'forgetPassword'">
					<el-button v-if="isForgetPass" style="width:100%" type="primary" :disabled="(!isdata)"  @click="_formTel()">下一步</el-button>
					<el-button v-else style="width:100%" type="primary" :disabled="(!isdata || !ruleForm.password || !isdata2 || !ruleForm.grapCode || !ruleForm.phoneCode || !ruleForm.statementState)" @click="_savePassword()">保存密码</el-button>
				</template>
	...
	data () {
		var validatePassPhone = (rule, value, callback) => {
			...
			if (value === '') {
				...
				// let pattern = /^[1](([3][0-9])|([4][5,7,9])|([5][0-3,5-9])|([6][5,6])|([7][0-8])|([8][0-9])|([9][1,8,9]))[0-9]{8}$/;
				// let pattern = /^((1[35678][0-9])|(14[57])|(17[0678])|(19[7]))\d{8}$/;
				let pattern = /^\d{6,16}$/;
		...
		*/
		...
		_noteAisle () {
			...
			this.$ajax({
				...
				if (res.success) {
					this.ruleForm.phoneCode = 123456;
					this.$message({
						type: 'success',
						message: res.message
					});
